...
- 双栏工作台布局；移动端自适应为页签切换 + 底部详情弹层
- 点击结果项在右栏内滑入详情卡片（替换统计卡）：文件列表、大小、创建日期、tracker 列表、info hash；点击其他区域收回
- 转换结果与历史列表均支持分页（首页/上/下/尾页 + 每页 10/20/50/100 条）
- 公共 tracker 注入（约 20 个）与输出格式选项（含/不含 dn、tr；hash hex/base32），实时作用于所有输出
- 每条结果支持复制 / 一键打开（magnet: URI）/ Web Share 分享（环境不支持时自动隐藏）
- 一键复制全部 + 复制 info hash
- 视频过滤：仅显示包含视频文件的磁力链接，含过滤摘要（右栏统计卡下方）与后缀正则；后缀标签和正则均点击即复制
